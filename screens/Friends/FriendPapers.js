...
class FriendPapers extends pickableImage{
  constructor(props) {
    super(props);
  ...
  getProfile = async() => {
      ...
      return fetch("http://13.124.127.253/api/results.php?page=getUserPaper&seq=" + this.state.friendId)
  ...
  render() {
    ...
    return(
      <TouchableOpacity style={[styles.grayBottom,styles.row,{justifyContent:'space-between',padding:13,paddingBottom:8}]}
        onPress={() => {this.props.hasPaper && this.props.setModalVisibleParent(true,this.props)}}
      >
        ...
        <View>
          <Text style={{fontSize:12,color:this.props.hasPaper ? '#666' : '#db3928'}}>{this.props.hasPaper ? "보기" : "없음"}</Text>
        </View>
      </TouchableOpacity>
    )
  }
